...
const pipe_interval = 60					'next pipe interval frame - pipe_rnd_interval
...
const as boolean cross_mutated_get_new_ID = true
...
do
	...
	if stats_counter = 0 then '*part 2 after respan
		...
		'line stats_image, (graph_scr_x - 1, graph_scr_y - 1)-(graph_scr_x - 1, graph_scr_y - 1 - (old_highest_fitness_alive/50)), rgb(0, 0, 127)
		'line stats_image, (graph_scr_x - 1, graph_scr_y - 1)-(graph_scr_x - 1, graph_scr_y - 1 - (avg_top/100)), rgb(63, 63, 63)
		'circle stats_image, (graph_scr_x - 1, graph_scr_y - 11 - (cels.ma(old_highest_fitness_alive/50, 50))),.5 , rgb(63, 63, 63),,,,f
		for i as ushort = 0 to list.active.lengte
			found = list.IsInList(List.active.ID(i), list.top)
			if found <> -1 then
				fitness.MA(list.plot(i).MA(ai.fit_selection(found).fitness, 50), ai.fit_selection_counter)
				circle stats_image, (graph_scr_x - 1, graph_scr_y - 1 - (list.plot(i).ReturnSom/50)), 1 , nummerToRGB(list.active.ID(i)),,,,f
			end if
		next i
		circle stats_image, (graph_scr_x - 1, graph_scr_y - 1 - (fitness.ReturnSom/50)), 2, rgb(255, 255, 255),,,,f
	...
	if graph_vsync_on then
		...
		draw string (graph_scr_x / 2 - 50, graph_scr_y / 2 + 70),"<Press S > pauze", &h0FFF00
		draw string (graph_scr_x / 2 - 50, graph_scr_y / 2 + 80),"<Press m > disabel mutate", &h0FFF00
	end if
...
sub gen.select_fittest_sortFast()
	static as ushort LookupTable(gen_size, 1) '0= index, 1=fitness
